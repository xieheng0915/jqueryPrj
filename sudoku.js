...
var col1 = "#B00";　//red
var col2 = "#0A85FF"; //blue

//generate random numbers then push into the array as the initial cell numbers
...
// Set cell (y,x) with value n   (0 for empty cell), set user select panel.
...
//filter used number on horizontal, vertical, oblique, return the left as the selection options for panel
...
//computer by cell, check allowed numbers for each cell
function bestHypothesis(A) {
    ...
    //return allowed number: bAll, bSc:count number, bCoords: each cell
}

//shuffle number, find the allowed numbers, then put allowed numbers into T array 
...
//put T array to Tsol array,Tsol: solution arr
...
function _getRandomGrid(nlevel) {
    ...
    }// only blank cells are set as clickable
    // make clickable
    $( "#waiting" ).popup( "close" )
    //$.mobile.loading().hide();
    hyp = false;
